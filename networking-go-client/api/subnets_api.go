...
func (api *SubnetsApi) ListAwsSubnets(xClusterId *string, filter_ *string, orderby_ *string, select_ *string, args ...map[string]interface{}) (*import4.ListAwsSubnetsApiResponse, error) {
	argMap := make(map[string]interface{})
	if len(args) > 0 {
		argMap = args[0]
	}

	uri := "/api/networking/v4.0.b1/aws/config/subnets"
	...
	unmarshalledResp := new(import4.ListAwsSubnetsApiResponse)
	json.Unmarshal(responseBody.([]byte), &unmarshalledResp)
	return unmarshalledResp, err
}
