...
// Create a new Storage Container with the specified configuration on the Cluster.
...
// Get an existing Storage Container's configuration by {containerExtId}. Note that Storage Containers from PEs having versions prior to 6.5 might have missing attribute data. Also Storage Container on these PEs under Self Owned RBAC category might not be visible for non-admin users.
...
// Query for stats information which is associated with the Storage Container identified by {containerExtId}.
...
// Lists the Storage Containers present on the system. Note that Storage Containers from PEs having versions prior to 6.5 might have missing attribute data. Also Storage Container on these PEs under Self Owned RBAC category might not be visible for non-admin users.
...
// Update the configuration of an existing Storage Container identified by {containerExtId}.
